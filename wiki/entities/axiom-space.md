...
- **Optical-relay-first.** ODC Node 1 & 2 carry **2.5 Gbps optical inter-satellite links** compatible with the **SDA Tranche-1** optical-comms standard, routing through Kepler's LEO optical-relay constellation rather than a single bent-pipe downlink. Kepler launched **10 optical-relay satellites on 2026-01-11** (Falcon 9, Vandenberg), each ~300 kg with **≥4 optical terminals + multi-GPU compute + terabytes of storage**; a **second tranche introduces 100-Gbit optical** with full backward compatibility — a ~40× step over the current 2.5 Gbps once flown. This makes Axiom the clearest commercial proof that the load-bearing ODC technology is **optical ISL** ([[concepts/leo-value-chain]] midstream A), not just the compute node — the same chokepoint flagged on [[synthesis/orbital-data-center-six-region]]. Note the flown-vs-bench split there: Axiom/Kepler's **2.5 Gbps is the US's fastest *flown* ODC optical link** (100-Gbit tranche pending), below China's multi-day [[entities/ada-space]] laser-ISL endurance demo and Google [[entities/google-suncatcher]]'s 1.6 Tbps *bench* ceiling.
