---
type: entity
tags: [person, economist, governance, web3, plurality, mechanism-design]
---

# E. Glen Weyl

US economist and social technologist; founder of the **RadicalxChange** movement and co-author with **[[entities/audrey-tang]]** of *Plurality* (2024). Originator (with collaborators) of the mechanism-design toolkit — **Quadratic Voting** and **Quadratic/Plural Funding** — that supplies the formal backbone of the [[concepts/plurality]] philosophy.

## Current roles (verified 2026-07-22)

| Vehicle | Role | Note |
|---|---|---|
| **Microsoft Research** | Research Lead of the **Plural Technology Collaboratory** | his institutional day-job; the industrial-research base from which the *Plurality* agenda is funded and prototyped |
| **Plurality Institute** | Co-founder & President / Chair | the non-profit that carries the *Plurality* book's research program forward |
| **RadicalxChange Foundation** | Co-founder | the movement organization (2018–) deploying QV/QF and "plural" social-tech; positions itself as "advancing participatory governance for the age of AI" |
| **Faith, Family and Technology Network** | Co-founder | vehicle for his 2026 pivot to faith-and-technology (see below) |

**2026 activity signals.** Weyl's work has moved decisively toward *AI governance* framing: on **2026-07-06** he convened/spoke at "**Can AI See Me? Human Dignity, Identity, and Moral Agency in Global AI Governance**," an online side event of the **UN Global Dialogue on AI Governance** (Geneva, 6–7 July 2026) — placing the *Plurality* thesis (human agency + plural participation over centralized AI control) inside a UN-level process. A distinctive 2026 turn is the **faith-and-technology** agenda ("Technology for Religious Empowerment" at Microsoft Research), extending the plural-diversity argument to religious communities' flourishing in the digital age.

## Why he matters here

The wiki's [[concepts/plurality]] cluster previously cited only Audrey Tang; Weyl is the co-author and the source of its *quantitative* half. He is the intellectual link between three worlds: academic mechanism design, the Ethereum public-goods ecosystem (Vitalik Buterin, Gitcoin), and Taiwan civic-tech (Tang). Splitting him into his own node makes that bridge explicit instead of buried.

## Key contributions (layer-down)

| Work | Year | Significance |
|---|---|---|
| *Radical Markets* (with Eric Posner) | 2018 | Introduced **Quadratic Voting (QV)** and the "common ownership / Harberger tax" ideas to a wide audience |
| *"Liberal Radicalism"* (with Vitalik Buterin & Zoë Hitzig) | 2018 | The **Quadratic/Plural Funding (QF)** paper — the math behind **Gitcoin Grants** and most crypto public-goods matching |
| **RadicalxChange** | 2018– | Non-profit/movement deploying QV/QF and "plural" social-tech globally |
| *Plurality* (with [[entities/audrey-tang]]) | 2024-05-20 | Synthesizes deliberation tech + plural funding into a single governance philosophy |

He is described as a principal collaborator of Vitalik Buterin (Ethereum), Jaron Lanier, and Audrey Tang.

## Six-region note

Weyl is the **US**-side intellectual anchor of the plural-governance field; [[entities/audrey-tang]] is the Taiwan-side practitioner anchor. The QF mechanism has its largest live deployment in the (globally distributed, US-led) Ethereum/Gitcoin ecosystem; QV experiments have appeared in US state legislatures (Colorado) and European municipal budgeting. The *Plurality* framework has now also crossed decisively into a foreign legislature via Japan's **Team Mirai** (Takahiro Anno, 1 House-of-Councillors seat 2025 → **11 House-of-Representatives seats 2026-02**) — an export path detailed on [[entities/audrey-tang]] and the [[synthesis/digital-democracy-user-owned-social-six-region|six-region map]].

> **Update — Gitcoin's plural pivot (verified 2026-09-08).** The largest live QF deployer has itself gone *more* plural: Gitcoin sunset the Grants Stack (May 2025) and launched **"Gitcoin 3.0"** with **GG24** (Oct 2025) — a multi-mechanism funding model over six thematic domains in which QF is now one instrument among several, not the whole product; Sybil tooling moved to **Human Passport**. Weyl's QF idea thus survives as a *component* of a broader plural-funding stack rather than as the singular mechanism — arguably the *Plurality* thesis validating itself against its own most famous instrument.

## Historical lineage & long-horizon view

**Lineage (back three decades).** Weyl's toolkit descends from a long line of mechanism-design work on protecting minority preferences against simple-majority rule: Vickrey–Clarke–Groves (1961–73) incentive-compatible mechanisms → the "quadratic" cost-of-votes idea Weyl formalized in *Radical Markets* (2018, with Eric Posner) → the QF matching formula (Buterin, Hitzig & Weyl 2018) that Gitcoin turned into the dominant crypto public-goods funding method. The intellectual arc runs from **market-design theory → crypto public goods → participatory AI governance**, each stage broadening the constituency for the same core claim: *widen the space of collective choice rather than collapse it to one price or one vote.*

**Long-horizon view (scenario — not fact).** Weyl's century bet is that the primitives he formalized (QV, QF, plural credentials) become standard civic infrastructure for an AI-saturated public sphere — but, exactly like [[concepts/plurality]] and [[concepts/dsnp]], every mechanism collapses without robust [[concepts/proof-of-personhood]] (one actor minting a thousand identities breaks QV and QF alike). His 2026 pivots — into UN-level AI-governance dialogue and into faith-and-technology — read as an attempt to build the *legitimacy and coalition* layer that the mechanisms need to survive capture, whether by platforms (the risk [[entities/project-liberty]] fights) or by centralized AI. His AI-governance turn is the intellectual sibling of [[concepts/ai-mediated-deliberation]]: the same *Plurality* logic (widen collective choice, don't collapse it) now applied to the LLM layer that both threatens discourse and can mediate it.

## Sources

- **[2026-07-22 verify]** Current roles (MSR Plural Technology Collaboratory, Plurality Institute, Faith/Family/Technology Network) + UN Global Dialogue "Can AI See Me?" 2026-07-06 — [RadicalxChange](https://www.radicalxchange.org/updates/announcements/2026-07-can-ai-see-me/), [glenweyl.com](https://glenweyl.com/)

## Related

- [[concepts/plurality]] — the book and philosophy he co-authored
- [[concepts/ai-mediated-deliberation]] — the LLM-era deliberation frontier his AI-governance turn parallels
- [[entities/audrey-tang]] — co-author and practitioner counterpart
- [[concepts/proof-of-personhood]] — QV/QF require Sybil-resistant personhood to function
- [[entities/project-liberty]] — funds Plurality-aligned research
- [[concepts/dsnp]] — the user-owned-graph protocol the Plurality movement's social-layer bet runs on
- [[synthesis/digital-democracy-user-owned-social-six-region]] — the six-region map he anchors the US Axis-B node of
- [[sources/plurality-audrey-tang-sampras-2025]]
