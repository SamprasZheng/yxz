...
**Substrate-layer refresh (2026-09-21):** added the *compute substrate* under the on-board-autonomy archetype — a dimension the map under-weighted. Europe is bidding to own it sovereignly: [[entities/axelera-ai|Axelera AI]] (Dutch in-memory-compute inference AIPU) partnered with [[entities/aiko-space|AIKO]] (2026-02-09) to run `OLIVER`/`GENE` on a European chip instead of NVIDIA Jetson, and with **ESA (2026-03-05) to standardize space-based inference** — the substrate analog to China's Huawei-Ascend sovereignty vs the US/Japan/Korea/Taiwan default of NVIDIA Jetson (Orin on [[sources/navi-orbital-vlm-earth-obs-2026|NAVI-Orbital]], [[entities/liscotech|LiscoTech]]). AIKO's [[entities/aiko-space|ASIMOV]] program (ASI-funded, *non-cooperative* RPO mapping) decoded + consortium logged in [[concepts/satellite-digital-twin]]. Entity fact-checks: [[entities/cognitive-space]] NOAA award corrected to the **2024-04-10 $900K GPD** contract + funding-figure discrepancy flagged; [[entities/msbai]] re-confirmed no new award since the Sept-2025 SBIR.
