...
|---|---|---|---|
| **Mem0** | LLM fact-extraction → vector store + entity-linking; add recall *without* replacing the runtime; managed or self-hosted | US | Largest agent-memory community; **~$20M Series A (Oct 2025), ~$20.5M total; ~186M API calls/quarter** <!-- 2026-09-11: prior "$24.5M raised" softened — Startup Intros / Crunchbase-type trackers report $20M Series A (Oct 2025) + $20.5M total; the $24.5M figure is unverified against a primary round announcement --> |
...
- **MCP (Model Context Protocol)** — *vertical*, agent↔tool/data ("USB-C for tool connectivity"). This is the standard [[concepts/nemoclaw|NemoClaw]]'s L7 egress policy and [[concepts/hermes-agent-framework|Hermes]]' `:8642` endpoint already speak; see [[concepts/mcp-aerospace-applications]] for the aerospace tool-server instance **and the canonical governance detail**. Anthropic-initiated (Nov 2024), latest spec **2025-11-25**, ~**110M monthly package downloads** by 2026 — the most widely adopted project in its foundation.
- **A2A (Agent2Agent)** — *horizontal*, agent↔agent across org boundaries ("HTTP for agent collaboration"). Announced by Google 2025-04-09, **donated to the Linux Foundation 2025-06-23**, reached **v1.0 in April 2026** with **150+ supporting organizations**, deep integration across Google/Microsoft/AWS, and production deployments in supply-chain, finance, insurance and IT-ops.

**Governance consolidated in Dec 2025 (the fact that sharpens the convergence read).** The two standards are no longer merely "both somewhere under the Linux Foundation": on **2025-12-09** the LF formed the **Agentic AI Foundation (AAIF)**, a directed fund anchored by three founding project contributions — **MCP (Anthropic), goose (Block), and AGENTS.md (OpenAI)** — and **co-founded by Anthropic + OpenAI + Block**, with Amazon, Bloomberg, Cloudflare, Google and Microsoft at the top tier; within ~4 months it passed **170+ member organizations**. A2A continues as its own LF project alongside AAIF. The signal is not the org chart but *who sits on it*: the closed-frontier rivals ([[synthesis/open-weight-llm-agent-stack-six-region|Anthropic and OpenAI]]) chose to **co-govern the agent-interop primitives they compete above** — the strongest 2026 evidence yet that the interop layer converges rather than fragments. (This repo's own `AGENTS.md`/`CLAUDE.md` convention is an instance of one of those three anchored standards.) Canonical governance detail lives on [[concepts/mcp-aerospace-applications]]; this hub only summarizes it.

A production system runs **both**: A2A routes a task to the right specialist agent, MCP gives that agent its tools and context. **The 2026 evidence leans toward *convergence*, not fragmentation** — three neutral standards under one rival-co-governed foundation, adopted by all three US hyperscalers — which weakens the "vendor-captured runtime" arm of the fork at the *protocol* level. The capture risk that remains is not the wire protocol but the **memory store** (above): you can carry an A2A/MCP-speaking agent between vendors, but not the accumulated per-user skill/memory unless *that* is portable too. So the fork's real battleground has narrowed from "will the protocols interoperate" (increasingly yes — now with a named rival-co-governed foundation behind them) to "will your agent's *memory* be portable" (unsettled — no equivalent foundation exists for the memory store).
...
- **~2030s** — Interop standards (MCP, A2A) win or fragment. **2026 evidence leans win — and hardened in Dec 2025:** the LF's rival-co-governed **Agentic AI Foundation** (Anthropic + OpenAI + Block) now anchors MCP/goose/AGENTS.md, and A2A (150+ orgs across all three US hyperscalers) sits beside it (see the interop-axis section above). If they keep winning, frameworks commoditize like web frameworks did and the *sandbox + memory* become the paid layer — which is precisely why the dedicated memory market (Mem0/Letta/Zep) capitalized in 2026, *without* an equivalent neutral foundation. If they fragment, model vendors capture runtimes (China's funnel model spreads).
...
| Claim in this map | What would falsify it | Status (2026-09-11) |
|---|---|---|
...
| The framework is a funnel, the memory is the moat | Frameworks stay differentiated and profitable while memory/skill stores commoditize | **Strengthening** — a *dedicated* US memory market (Mem0 ~$20.5M total / ~186M calls-per-qtr, Letta ~$10M seed, Zep) capitalized *underneath* the frameworks; portability of the store, not the protocol, is now the open question — and unlike MCP/A2A the memory store has **no vendor-neutral foundation** (no memory analog to the 2025-12 AAIF), so capture risk concentrates here |
...
| Interop standards converge (not fragment) | MCP or A2A splinters into incompatible vendor forks, or a hyperscaler ships a proprietary rival | **Strengthening toward convergence** — the LF formed the rival-co-governed **Agentic AI Foundation (2025-12-09)** anchoring MCP + goose + AGENTS.md (Anthropic + OpenAI + Block co-founders, 170+ orgs by ~2026-04); A2A hit v1.0 (2026-04) with 150+ orgs + Google/MS/AWS integration; no dominant proprietary rival has emerged. The closed-frontier rivals *co-governing* the interop primitives is the year's strongest convergence signal |
...
- Agent-memory market: Mem0 (~$20M Series A Oct 2025 / ~$20.5M total, ~186M API calls/quarter, largest community), Letta (~$10M seed Sep 2024, memory as first-class agent component, core+archival blocks), Zep (temporal knowledge-graph); ~65% of enterprise-agent failures attributed to context drift — Mem0.ai, DEV Community, Braintrust, Developers Digest, Value Add VC, Startup Intros

**Refreshed 2026-09-11 (this deepening pass):**
- **Agentic AI Foundation (AAIF)** formed by the Linux Foundation **2025-12-09**, anchored by MCP (Anthropic) + goose (Block) + AGENTS.md (OpenAI); co-founded by Anthropic + OpenAI + Block, top-tier members Amazon/Bloomberg/Cloudflare/Google/Microsoft; **170+ member orgs within ~4 months**; MCP its most-adopted project — Linux Foundation press (2025-12-09), Oracle AI & Data Science blog, AgentEngineering, Agentic-AI Knowledge Base. (Canonical governance detail: [[concepts/mcp-aerospace-applications]].)
- MCP: latest spec **2025-11-25**, ~**110M monthly package downloads** by 2026; A2A reached **v1.0 April 2026** (150+ orgs, Google/MS/AWS) — DEV Community (MCP vs A2A 2026 guide), Wikipedia Agent2Agent
- Hermes Agent GitHub stars **~244k as of 2026-09-11** (from ~188k Jun → ~224.6k early Aug), still fastest-growing OSS agent framework; current release line v0.21.x (Sep 2026) — Dealroom, Gradually.ai Hermes-Agent statistics, GitHub NousResearch/hermes-agent releases
- LangChain/LangGraph 1.0 (GA 2025-10-22) adoption re-confirmed: ~100M+ monthly downloads / 139k★ (LangChain), LangGraph ~34.5M downloads/mo across production shops (Uber/LinkedIn/Klarna/Replit/Elastic/Cisco); LangChain "State of Agent Engineering" reports 57% of 1,300+ respondents have agents in production — langchain.com/blog, langchain.com/state-of-agent-engineering
