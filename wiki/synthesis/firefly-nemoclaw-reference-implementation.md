...
> **Re-verified against the `agents/` tree on 2026-08-06** (code last changed at commit `7817a4d`, 2026-07-13). The headline finding of that pass: the orchestration divergence documented on 2026-06-14 has **inverted** — the Python runtime-of-record now runs Nemotron end-to-end, and the lone `claude-opus-4-7` declaration that remains is in `nemo_workflow.yaml`, which now *lags* the code rather than driving it (full detail in the divergence section below).
>
> **Re-re-verified 2026-09-11** against the current tree (the llm code + both YAMLs were last touched 2026-08-18, *after* the 08-06 check, so a re-read was warranted). **All prior findings hold, nothing regressed:** `src/firefly/llm/router.py` + `nemotron.py` confirm the default local backend serves `nemotron-3-nano:4b` for **both** the planner (thinking ON, temp 0.15) and executor (thinking OFF, temp 0.3) tiers via Ollama at `127.0.0.1:11434/v1`, with cloud SKUs Super-49B (planner) + Nano-9B-v2 (executor) only under `NEMOTRON_BACKEND=nim`, and a `disabled` backend for graceful degradation; `orchestrator.py`'s docstring still declares *"Architecture (Nemotron dual-model split)"* and injects one `LLMRouter()` into every agent's `context`; **`grep -rn claude src/` returns nothing** — Claude is in no `.py`. **The one open item is unchanged and now ~1 month stale: `nemo_workflow.yaml` lines 18 + 37 still declare `claude-opus-4-7`** (the 08-06-recommended one-line doc fix has not been applied). Re-flagged for the owner below.
