...
| 1024-QAM | **≈1.8%** (industry *extrapolation*, not a distinct spec cell — see resolved note below) | calibration-and-converter-limited regime; demands phase noise <~0.3° RMS + sub-0.1 dB / sub-0.5° IQ calibration |

> **Fact-check resolved 2026-09-14** (supersedes the 2026-08-24 flag "the exact TS 38.104 Table 6.5.2.2-1 cell was not primary-verified"): the standing ambiguity is now settled — the two claims must be split. (1) **1024-QAM *support* is ratified**: it is an *optional downlink* modulation added in **3GPP Release 17**, and to date deployed only in FR1 fixed-wireless-access / controlled-environment links, not mobile. (2) **The ≈1.8% EVM number is NOT a ratified spec cell.** Table 6.5.2.2-1 (BS transmitted-signal quality) tops out at the 256-QAM = 3.5% row; the ≈1.8% figure is an *industry extrapolation* of the QPSK→256-QAM halving sequence (Keysight/R&S/RF-Essentials), consistently reported but not a distinct 3GPP-defined ceiling. Prior text ("ratified ... EVM ceiling of ≈1.8%") conflated the two and slightly overstated the second; corrected here. The dated primary marker that 1024-QAM has moved from *spec option* to *certifiable-in-lab*: **Keysight gained the first GCF-recognized validation of 5G NR FR1 1024-QAM demodulation test cases on 2024-07-29** — i.e. the T&M tier (sub-layer C) now formally *measures* the order even where the transmit-EVM cell is extrapolated. The structural point is unchanged and sharper: the highest fielded order sits *below* 2% EVM, squarely in the converter-linearity/phase-noise-limited regime where sub-layers A and C bind before the PA does — and, tellingly, the region that owns the instrument (US/Europe, sub-layer C) is the one that *defines the test case* before any spec cell exists.
...
**The direct-RF boundary is still moving toward the antenna (dated, 2026-08-24):** AMD's **Versal RF Series** (announced 2024-12-10; the **5th generation** of AMD direct-RF devices, succeeding the Zynq UltraScale+ RFSoC above) integrates 14-bit RF-ADCs sampling to **32 GSPS and reaching ~18 GHz** on-chip — roughly triple the Gen-3 rate and into Ku-band without a downconverter — alongside **80 TOPS of DSP** in hard-IP + AI-Engine fabric (up to ~19× the Gen-3 channelizer compute) and hard blocks for FFT/channelizer/LDPC. **Ramp status re-verified 2026-09-14:** AMD is *now shipping* the first **VR1602** engineering samples (per AMD's own "Now Shipping" blog), with the larger **VR1652** to follow **Q1 2026** and volume **production still expected H1 2027** — the timeline holds, one quarter into the sampling window. Two structural reads: (1) the analog/certification burden keeps migrating up-frequency, so the region owning these converters (US, sub-layer A) extends its lead into Ku/Ka SATCOM bands; and (2) folding the linearization/beamforming DSP and the *converters* into one adaptive SoC is the concrete face of the RF↔AI-compute convergence — sub-layer B ([[concepts/dpd-digital-predistortion|DPD]]) increasingly runs on the same silicon that digitizes the RF.
...
|---|---|---|---|---|
| **US** | **Leads** — Analog Devices (transceivers + RF-sampling ADC/DAC), AMD/Xilinx (RFSoC Gen-3 → **Versal RF** Gen-5, 32 GSPS/~18 GHz, prod. H1 2027), TI, Broadcom | **Leads** — ADI (ADRV904x integrated DFE), Broadcom (*BroadPeak* 5 nm 6G DFE, 19.6 GS/s, announced 2026), AMD/Xilinx DPD IP (now co-resident with converters + AI-Engine DSP on Versal RF) | **Co-leads** — Keysight (89600 VSA, Infiniium), Tektronix | **Only region owning all three** — can self-certify end-to-end |
...
**Market anchor for the instrument chokepoint (C)** (re-verified 2026-08-24, unchanged): Keysight + Rohde & Schwarz + Anritsu hold **>50%** of the merchant vector-signal-analysis market (≈**$422.9M in 2025**, ~7.1% CAGR to 2033; Keysight ~27% / R&S ~19% of the adjacent VNA volume; the broader spectrum-and-signal-analyzer market ≈$1.48B in 2026 → ~$2.20B by 2035 gives the same three-vendor concentration a larger denominator). Sub-layer C is deliberately the last column because it is the narrowest and least substitutable: calibration *know-how* (the LO-leakage ternary search, the IQ pre-distortion matrix, GMP/NN-DPD identification) is **portable IP that travels in a person's head**; the instrument that measures whether the calibration worked is a capital good only three firms make well.
...
- If direct-RF sampling displaces zero-IF in dense LEO consumer arrays (converter power-per-GSPS falls enough) → sub-layer A's "zero-IF is the cost optimum" framing shifts, and LO-leakage/IQ calibration fades as a discipline faster than projected. **Watch the Versal RF Gen-5 ramp (production H1 2027) and its published power-per-GSPS**: a Ku-band-reaching direct-RF SoC at consumer-array power would be the concrete trigger.
...
- External primary anchors (fact-check 2026-09-14): [Keysight — first GCF-recognized validation of 5G NR FR1 1024-QAM demodulation test cases (2024-07-29)](https://www.keysight.com/us/en/about/newsroom/news-releases/2024/0729-pr24-108-keysight-gains-first-validation-of-5g-new-radio-fr1-1024-qam-demodulation-test-cases.html); [AMD — Now Shipping: Versal RF Series (2025)](https://www.amd.com/en/blogs/2025/now-shipping-versal-rf-series.html)
