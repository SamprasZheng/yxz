...
- The "dilution region" (high miss distance, poor covariance) requires special handling — CARA tags these events separately. See the dedicated mechanism section below.

## Layer Down: The Dilution Region and Maximum Pc (加深探究)

A low Pc is **not** the same as a safe conjunction — and the reason is the single most important subtlety in the whole metric. Because Pc is the *overlap* between the collision disk and the position-error PDF, it goes to zero at **both** extremes of uncertainty:

- **Tiny covariance** (very precise orbits, miss vector outside the disk): the PDF is a sharp spike missing the disk → Pc ≈ 0 because you *know* the objects miss.
- **Huge covariance** (very poor orbits): the PDF is spread thin over a vast area → the fraction inside the finite collision disk is negligible → Pc ≈ 0 because you *don't know anything*.

Since Pc → 0 at both ends, as a function of covariance magnitude **it must rise to a single maximum somewhere in between**. Scaling the combined covariance by a positive factor *k* (C → *k*C) and recomputing traces out this curve; its peak is the **maximum probability of collision (max Pc)** — the largest Pc consistent with the encounter geometry (miss vector + combined [[concepts/hard-body-radius|HBR]]) over *all* covariance magnitudes. Alfano derived this relation and the "unit-aspect-ratio" spherical bound; Frisbee's refinement uses the reported covariance *shape* and scales it, implemented in NASA's public **Single Covariance Maximum Pc** tool (GSC-17900-1). ([Alfano 2005, *J. Astronautical Sciences*](https://link.springer.com/article/10.1007/BF03546350); [NASA Single Covariance Maximum Pc](https://software.nasa.gov/software/GSC-17900-1))

**The dilution region** is the regime where the *actual* covariance is larger than the max-Pc covariance — i.e. the operating point sits on the **falling right-hand tail** of the Pc-vs-covariance curve. There, a low reported Pc is an artifact of *ambiguity*, not of demonstrated safety: making the covariance even worse would drive Pc *lower still*. The standard deviation associated with the max-Pc scaling **defines the boundary of probability dilution** — inside it, Pc is trustworthy; beyond it, "the Pc value is depressed to a low value not because of high certainty but rather because of large ambiguity in the state estimates." ([NASA CARA, NTRS 20190029216](https://ntrs.nasa.gov/citations/20190029216))

Two operational corollaries:

1. **Max Pc is an upper-bound guard, not a decision metric.** CARA's own finding is explicit: **"Maximum Pc is not sufficient as a stand-alone [risk-assessment] parameter"** — a near-miss geometry can force max Pc to 1.0 while the *actual* covariance-conditioned Pc is tiny, so triaging on max Pc alone would maneuver on nearly every close pass. Its correct use is as a **dilution flag**: if max Pc is high but Pc is low *because the covariance is wide*, the event is un-actionable-but-not-safe → hold a **presume-risky posture** and wait for the covariance to tighten (which it does as [[concepts/tca-time-of-closest-approach|TCA]] approaches) rather than clearing it green. ([NASA CARA, NTRS 20190029216](https://ntrs.nasa.gov/citations/20190029216))
2. **This is a live, unsettled research front, not a solved corner case.** The dilution flaw motivated the Balch–Martin–Ferson **"false confidence theorem"** critique (a large covariance can make a genuinely dangerous conjunction *look* safe — a structural, not numerical, failure of Pc) and NASA CARA's 2023 program to **replace the Pc metric** outright with an inference-on-miss-distance formulation. The math of the integral is settled; whether Pc is even the *right* risk statistic in the dilution regime is not. ([Balch/Martin/Ferson, false-confidence theorem](https://arxiv.org/abs/1706.08565); [NASA "Setting the Bar for the Replacement of the Pc Metric," NTRS 20230018683](https://ntrs.nasa.gov/citations/20230018683))

> **Dated real-world anchor (2026):** on **2026-03-20 06:23 UTC**, STARLINK-36658 and the operational Chinese EO satellite SITRO-AIS 37 had a predicted **minimum range of ~9 m** with a reported **maximum collision probability of 1.0** — one of nine Starlink conjunction threats logged over 2026-03-20→24, one rated HIGH. A max Pc pinned at 1.0 is the metric saturating on geometry alone (a 9 m miss is inside almost any plausible combined HBR + covariance), which is exactly why max Pc must be read as an *upper-bound guard* against dilution, not as the maneuver trigger — the maneuver decision still turns on the covariance-conditioned Pc and the cost–loss terms in the next section. ([FODNews, 2026-03](https://fodnews.com/starlink-nine-conjunction-threats-march-2026-leo-congestion/); media report of tracking data, not an independent primary read — flagged.)

For the Firefly / Mission Desk agent, this adds one non-negotiable rule to the triage record: **never emit "green" on a low Pc when the covariance is wide or absent.** Detect the dilution region (miss distance ≫ covariance-implied scale, or secondary covariance missing in `cdm_public`), carry a `dilution_region` flag + the max-Pc upper bound alongside the reported Pc, and default to the presume-risky, abstain-and-monitor posture argued in [[concepts/calibrated-confidence-llm]]. This is the conjunction-domain instance of the trust-stack's abstention principle: a confident low number built on unreliable inputs is worse than an honest "unknown."
