...
**Repo:** https://github.com/NousResearch/hermes-agent — MIT License, Python 88.6%. **Public launch 2026-02-25**; growth trajectory ~40k (Apr) → ~99k (8 weeks post-launch) → ~188k (Jun) → ~224.6k (early Aug) → **~244k stars (2026-09-11)** — still the **fastest-growing open-source agent framework of 2026** and the single most-starred entry in the [[synthesis/agent-runtime-orchestration-six-region|six-region agent-runtime map]]. Current release line **v0.21.x (Sep 2026)**. A major accelerant: **NVIDIA selected Hermes as a reference runtime for its Nemotron 3 Ultra (~550B) model** (Dealroom, The Agent Report, DEV Community, GitHub). <!-- fact-refresh 2026-09-11: ~188k (Jun) → ~244k (Sep 2026) per Dealroom/Gradually.ai/GitHub releases; added v0.21.x line. Prior 2026-07-18 refresh had corrected ~165k(May)→~188k(Jun). -->
...
**Framework-native vs standalone memory (2026 market context).** Hermes makes memory a *framework primitive* — the learning loop, skill store, and pluggable backends ship inside the agent runtime. This is one of the two integration philosophies in the 2026 agent-memory market: the other is **memory-as-a-service**, sold *underneath* any runtime by a dedicated US startup layer that capitalized in 2026 — **Mem0** (~$20M Series A Oct 2025 / ~$20.5M total, ~186M API calls/quarter, largest community; LLM fact-extraction → vector store), **Letta** (ex-MemGPT, ~$10M seed Sep 2024; core-memory blocks in-prompt + archival memory the agent tool-calls into), and **Zep** (temporal knowledge-graph). Tellingly, several of these are *also selectable as Hermes backends* (`mem0` is in the provider list above), so the two philosophies interoperate rather than compete. Hermes' framework-native choice is the stickier moat — memory inseparable from the agent loop is harder to carry between vendors than a bolt-on service — which is exactly the "memory portability, not protocol interop, is the live lock-in question" thesis mapped in [[synthesis/agent-runtime-orchestration-six-region]] (the memory / skill-store sub-layer). Industry framing that **~65% of enterprise-agent failures trace to context drift, not model capability** is why this sub-layer became a funded market in 2026.
...
> **Nemotron-coupling update (2026-07-25 fact-check).** The Hermes↔Nemotron relationship hardened from "experimental option" to **first-class support** around the [[concepts/nemotron|Nemotron 3 Ultra]] launch (2026-06-04): (a) Hermes Agent is now **officially supported and fully supported for use with Nemotron** (no longer merely the `nemohermes` experimental alias); (b) **Nous Research was inducted into NVIDIA's "Nemotron Coalition"** of labs advancing open frontier models; (c) NVIDIA ships a dedicated **NemoClaw-for-Hermes-Agent blueprint** on build.nvidia.com whose **default model is `nvidia/nemotron-3-super-120b-a12b`**, with Nemotron reachable via a free build.nvidia.com key or a local NIM endpoint; (d) Nous publishes a **"Run Nemotron 3 Ultra free in Hermes Agent"** guide. Net: the mandated-for-the-hackathon US model ([[concepts/nemotron]]) and the fastest-growing runtime (Hermes) are now a vendor-blessed pair, which strengthens the [[synthesis/agent-runtime-orchestration-six-region|runtime-layer]] read that the durable moat sits at the runtime/memory layer while the model slots in underneath. Sources (accessed 2026-07-25): NVIDIA NemoClaw Hermes quickstart, build.nvidia.com NemoClaw-for-Hermes blueprint, hermes-agent.nousresearch.com run-Nemotron-3-Ultra guide.
...
- [[concepts/mcp-aerospace-applications]] — MCP-standardized tool connectivity layer; Hermes exposes an OpenAI-compatible (MCP-shaped) endpoint on port 8642 that aerospace tool servers can register against. MCP itself is now anchored by the **Agentic AI Foundation** (Linux Foundation, formed 2025-12-09, co-founded Anthropic/OpenAI/Block) — so a Hermes stack that speaks MCP is betting on a rival-co-governed neutral standard, not a single vendor's API; see the interop axis in [[synthesis/agent-runtime-orchestration-six-region]]
