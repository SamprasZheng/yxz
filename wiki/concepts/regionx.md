...
- **Refined 2026-08-01 — the burn is quantified, and RegionX is not the *only* marketplace, only the *trustless* one.** Two precisions this pass:
  1. **Burn rate (verified):** the coretime-revenue sink is codified in **[RFC-0010 "Burn Coretime Revenue"](https://polkadot-fellows.github.io/RFCs/approved/0010-burn-coretime-revenue.html)** — after the March-2026 upgrade, **~80% of coretime sales revenue** (plus a portion of transaction fees) is **burned** from circulation, tying the market directly to [[concepts/dot-hard-cap|DOT scarcity]]. A separate **Dynamic Allocation Pool (DAP)** governance change (~Jan 2026) redirected the *non-burned* remainder (revenue/fees/slashes) into a pool rather than destroying it — so the current split is *burn ~80% + DAP the rest*, not a 100% burn.
...
|---|---|---|---|
| **Polkadot coretime + RegionX** | reserved (bulk) + spot (on-demand) cores; **~80% revenue burned** (RFC-0010) | **yes — RegionX (trustless) + Lastic** | Polkadot (**Europe** — Web3F/Parity) |
