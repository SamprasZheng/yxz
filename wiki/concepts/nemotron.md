...
| **Nemotron 3 Ultra** | **2026-06-04** | **550B** MoE / **55B active** | up to **1M** | Deep-research / strategic planning; hybrid Mamba-Transformer MoE. **Now released** (was "~2026-Q2 / TBD" on prior revisions): the **most intelligent US open-weight model** at release (**Artificial Analysis Intelligence Index 47.7 on the then-current v4.1.1**) — ahead of the next US open-weight models Gemma 4 31B (39.2), Nemotron 3 Super (36.0) and gpt-oss-120b (33.3), but **a tier below the China-led open-weight frontier**. <!-- v4.3 update 2026-09-10: AA rebased the index (v4.2→v4.3, Sept 4, harder held-out evals + 40% private-test weighting → all scores ~15 pts lower; do not compare to the 47.7/60/63 v4.1.1 figures). On v4.3 the open leaders are Z.AI **GLM-5.3 ≈ 45** (open-weight #1, weights shipped 08-28) and Moonshot **Kimi K3 ≈ 44**; Nemotron 3 Ultra remains best-US-open but still sits below them, and the closed US frontier pulled further ahead with the GPT-6 Astra (09-03) + Claude Fable 5.1 (09-01) generation (both = 53 on v4.3). Full map: [[synthesis/open-weight-llm-agent-stack-six-region]]. --> Served **>300 tok/s** on a pre-release DeepInfra endpoint (peer China-lab 550B-class models DeepSeek/Kimi typically 50–100 tok/s) — the *speed-at-intelligence* is the differentiator, not the raw score. NVIDIA paired it with **[[concepts/hermes-agent-framework|Hermes Agent]] as a reference runtime** (Nous Research inducted into NVIDIA's **Nemotron Coalition**), coupling the model to the agent-runtime layer ([[synthesis/agent-runtime-orchestration-six-region]]) ([Artificial Analysis](https://artificialanalysis.ai/articles/nvidia-nemotron-3-ultra-released)) |
...
Nemotron is the **US national-champion open-weight line**, but it does not sit at the open-weight frontier. As of Sept 2026 (AA Intelligence Index **v4.3** — a Sept-4 rebase with harder held-out evals + 40 % private-test weighting, ~15 pts lower than the older v4.1.1 numbers; not cross-comparable) the open-weight intelligence frontier is **China-led** — **GLM-5.3 ≈ 45** (open-weight #1, weights shipped 08-28) and **Kimi K3 ≈ 44**, with GLM-5.3-Flash (42) / Qwen 3.8 (40) / DeepSeek V4 Pro 0813 (36) below — while the *closed* frontier stays US-led and just took a **generational step**: **Claude Fable 5.1 (09-01) and GPT-6 Astra (09-03) both = 53**, Claude Opus 5 = 51, GPT-5.6 Sol = 47. The best US open-weight, Nemotron 3 Ultra, sits below the China open leaders, and the open-vs-closed gap **reopened to ~8 pts (45 vs 53)** after mid-August's ~3-pt near-tie (the near-tie was on the old v4.1.1 ruler). Nemotron's role is therefore the **agentic-reasoning, NIM-deployable, license-clean** option a US/Taiwan team reaches for when it wants open weights + a vendor SLA — which is exactly why the [[sources/nvidia-agent-challenge-2026|GTC Taipei Agent Challenge]] mandates it. Taiwan's [[concepts/domain-specific-llm-agents|sovereign-model]] efforts (TAIDE 2.0) are now **migrating onto Nemotron** as their base, making NVIDIA the upstream of the Taiwanese model layer the same way TSMC is the upstream of everyone's compute. Full regional map: [[synthesis/open-weight-llm-agent-stack-six-region]].
