...
1. **算力**：商用 GPU（如 NVIDIA H100）需進行輻射防護或選用太空硬化元件；商用現貨（COTS）與太空硬化（Space-hardened）之間的取捨是最大工程挑戰。COTS GPU 最主要的輻射威脅是 [[concepts/see-single-event-effects]]（SEU 翻轉 SRAM 快取 + SEL 閂鎖燒毀），需通過 [[concepts/rha-radiation-hardening]] 認證流程（RDM ≥ 1.5）；see [[concepts/cots-gpu-radiation-risk]] for full risk assessment.
