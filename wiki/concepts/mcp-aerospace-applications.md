...
The Model Context Protocol (MCP) is an open standard (Anthropic-initiated, November 2024; latest spec 2025-11-25; contributed on **2025-12-09** to the **Agentic AI Foundation (AAIF)** — a directed fund under the **Linux Foundation** co-founded by Anthropic, Block, and OpenAI, anchored by three founding project contributions: MCP (Anthropic), goose (Block), and AGENTS.md (OpenAI), and grown to **170+ member organizations within ~4 months**; MCP is its most-adopted project at ~**110M monthly package downloads**) for connecting LLM agents to external tools and data sources over a uniform JSON-RPC-style transport. An MCP *server* exposes a set of typed tool descriptions; an MCP *client* (Claude, OpenClaw, Cursor, VS Code Copilot, etc.) discovers those tools and invokes them via natural-language reasoning. The key property is **model-agnostic tool inventory**: the same tool set can be driven by any MCP-compatible LLM without re-writing client code. MCP is used inside [[concepts/openclaw]] as its primary tool-connectivity layer, and [[concepts/hermes-agent-framework]] supports MCP-compatible endpoints. [[concepts/nemoclaw]] enforces L7 policy over outbound calls the sandboxed agent makes, which is architecturally complementary to MCP (NemoClaw polices egress; MCP standardises the tool-call shape). For a protocol-layer contrast, see [[concepts/agentic-payments]] — x402/ACP/AP2 operate at the *payment* layer; MCP operates at the *tool-invocation* layer. These are orthogonal and can compose.

**Governance maturity (verified 2026-07-07):** technical control sits with an **individual-based Steering Group** (Maintainers → Core Maintainers → Lead Maintainers), with membership attached to the person, not the employer, and *no company-reserved seats* — the same neutral-stewardship model the Linux Foundation uses for other critical infrastructure. In 2026 the Technical Steering Committee approved a formal **project-lifecycle policy (Growth / Impact / Emeritus)**, opening the foundation to external projects for the first time, and the MCP Dev Summit North America 2026 framed MCP as having crossed into "enterprise infrastructure." **AAIF membership refresh (2026-09-11):** the foundation passed **170+ member orgs within ~4 months of its 2025-12-09 formation** (top tier: Anthropic, OpenAI, Block, Amazon, Bloomberg, Cloudflare, Google, Microsoft) and MCP downloads reached ~**110M/month** — i.e. the neutral-stewardship bet has *strengthened*, not stalled, since the 07-07 check. For an aerospace tool owner this matters: publishing a SPICE/STK/Space-Track MCP server is now a bet on a **vendor-neutral, foundation-governed** standard rather than a single company's API — the durability argument for building the tool layer once.
