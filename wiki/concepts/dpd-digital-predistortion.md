...
|---|---|---|
| **US** | **Analog Devices** (ADRV904x integrated-DFE transceiver: DPD+CFR+CDUC/CDDC), **Broadcom** (*BroadPeak*, 5 nm DFE SoC, 400 MHz–8.5 GHz, ~40% power cut, "DPD learning 100× faster", 19.6 GS/s converters, 6G n104 band — announced 2026), **AMD/Xilinx** (FPGA DPD IP → **Versal RF** Gen-5: DPD/linearization DSP now co-resident with 32 GSPS RF converters + AI Engines on one die, prod. H1 2027), MaxLinear | **Leads** — deepest commercial DFE/DPD IP across transceiver, ASIC, and FPGA |
...
- **2026 (shipping now):** AMD's **Versal RF Series** (5th-gen direct-RF, production H1 2027) puts **80 TOPS of AI-Engine + hard-IP DSP on the same monolithic die as the 32 GSPS RF converters** — the concrete instance of "linearization runs on the fabric that digitizes the RF," collapsing the historical DAC → analog-mixer → separate-DPD-DSP chain into one adaptive SoC (see [[concepts/zero-if-transmitter]] lineage table).
- **2030s (projection):** per-element *online* NN-DPD that re-trains in orbit as the PA ages and temperature swings — the calibration table ([[concepts/rf-soc-debug-taxonomy|OTP/MCN-managed]]) becomes a live model rather than a one-time factory artifact. The CUDA acceleration above and the Versal RF AI-Engine fabric are the seed of this: the same GPU/AI-Engine compute that runs [[concepts/orbital-data-center|orbital data centers]] and [[concepts/nemotron|on-board reasoning models]] can host adaptive DPD, making linearization a software-update problem.
