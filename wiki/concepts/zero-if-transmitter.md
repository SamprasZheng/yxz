...
| ~2025→2027 | **Direct-RF pushes into Ku-band + folds in AI-Engine DSP** | AMD **Versal RF Series** (announced 2024-12-10; 5th-gen AMD direct-RF, succeeds the Zynq RFSoC): 14-bit RF-ADCs to **32 GSPS / ~18 GHz** on-chip + **80 TOPS** hard-IP/AI-Engine DSP (~19× the Gen-3 channelizer) with hard FFT/channelizer/LDPC blocks; samples Q4 2025, **production H1 2027**. The mixer-less boundary moves up-frequency (into Ku SATCOM) *and* the linearization/beamforming compute co-locates with the converters — the RF↔AI-compute convergence in one die |
...
|---|---|---|
| **US** | **Leads** — both zero-IF transceivers and RF-sampling converters; lead extends into Ku-band via Versal RF Gen-5 (32 GSPS/~18 GHz, prod. H1 2027) | Analog Devices (transceivers + RF-sampling ADC/DAC), AMD/Xilinx (Zynq RFSoC → Versal RF), Texas Instruments, Qorvo/Skyworks (front-end), Broadcom |
