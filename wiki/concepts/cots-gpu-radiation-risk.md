...
**The supply side is moving too (2022→2026 fact-check).** The heavy-ion-beam-hour bottleneck is not static: the US is visibly *adding* capacity via MSU's [[concepts/rha-radiation-hardening|FRIB]] — the **FSEE** heavy-ion SEE facility (live since Aug 2022, ~2,000 hr/yr, 41 corporate users) plus the **KSEE** K500 chip-test line (inaugurated 2026-02, up to 6,000 hr/yr), ~8,000 combined new beam-hours/yr, explicitly aimed at the microelectronics-for-spaceflight shortage. So the real question is a *race*: does new accelerator supply (FRIB + TAMU's widened lines) out-run the per-lot upscreen demand that COTS-everything creates? As long as constellation part-counts grow faster than beam-hours, the bottleneck holds even as capacity rises — the sovereignty read on [[synthesis/radiation-test-rad-hard-six-region]] survives the build-out.
...
| **Project Suncatcher** (first pair 2027, w/ **Planet Labs**; 81-sat 1 km cluster vision) | Google Trillium TPU v6e | **Ground beam test (only quantified public data):** 67 MeV proton; **no TID-attributable hard failure to 15 krad(Si)/chip**; **HBM = most sensitive subsystem**, first irregularities at ~2 krad(Si) ≈ **3× the shielded 5-yr mission dose of 750 rad(Si)**. Google published an orbital-**debris** risk assessment 2026-01 (still no on-orbit radiation data — none flown yet). | [Google Research](https://research.google/blog/exploring-a-space-based-scalable-ai-infrastructure-system-design/), [SatNews 2026-01](https://satnews.com/2026/01/19/google-addresses-orbital-debris-risks-for-project-suncatcher-ai-constellation/) |
...
**Reading:** the only *quantified beam* datum in public is Google's Trillium result, and it **confirms the wiki's standing thesis — HBM is the binding TID-sensitive subsystem**, degrading ~3× sooner than the logic. Starcloud's flight is an *existence proof, not a dataset*: no on-orbit error rates have been released, so the single most important number in LEO AI computing — the actual on-orbit SEU/SEL rate of a leading-node GPU — **remains unpublished** (the same data gap flagged in the Starcloud section above, now confirmed still-open one flight later). Next data points to watch: **Starcloud-2** (targeted **October 2026**, NVIDIA **Blackwell B200** + AWS server blade + bitcoin-mining ASICs, ~100× the power of Starcloud-1, carrying what Starcloud claims is the **largest deployable radiator ever flown on a private satellite** — the σT⁴ heat-rejection term made physical — running live workloads for AWS/Google Cloud/NVIDIA/Crusoe; see [[entities/starcloud]]) and the **Suncatcher × Planet Labs** on-orbit TPU pair (2027). Neither is expected to *release* on-orbit SEU/SEL/TID rates — so watch for whether commercial-workload SLAs force the first public error-rate disclosure.
...
| **2025–2026** | **Data-center-class COTS in orbit** — H100 (Starcloud-1, 2025-11), Trillium (Suncatcher × Planet Labs, 2027), Orin (Aitech), **B200 (Starcloud-2, Oct 2026)**, 200 kW/3-t Starcloud-3 (roadmap, Starship-sized) | ODC economics + terrestrial AI compute/power crunch; the "recompute is cheaper than rad-harden" bet |
...
|---|---|---|---|
| **US** | [[entities/starcloud]] (H100 flown → B200 Starcloud-2 Oct 2026 → 200 kW Starcloud-3 roadmap), [[entities/google-suncatcher]] (Trillium × Planet Labs 2027), [[entities/axiom-space]] (AxDCU) | **COTS + shielding + external SEL-cutoff + software ECC/retry** — "characterize-and-tolerate"; escalating deployable-radiator thermal design as node power climbs; **also hedging with a hardened-AI path** (Microchip PIC64-HPSC RISC-V, [[concepts/rha-radiation-hardening]]) | **Strongest** — only region publishing beam data (Google) *and* a flight existence proof (Starcloud); *also* the only region visibly expanding ground SEE beam-hours (FRIB) **and** re-onshoring rad-hard fab (BAE RH45, DoW DPA $16M 2026-07) |
...
- **A middle path is now materialising (2026 fact-check):** the fork is no longer purely "leading-node COTS GPU vs 20-year-old RAD750." The rad-hard IC line has begun its **first RISC-V + AI/ML generational leap in ~20 years** — **Microchip PIC64-HPSC** (NASA HPSC; 10-core RISC-V, SiFive X280 512-bit vector, ~100× legacy space-processor throughput) and **Frontgrade Gaisler GR765** (octa-core NOEL-V RISC-V / LEON5) — a *hardened* on-orbit-AI option that is deterministic and SEL-immune but **1–2 nodes behind and slow to certify** (PIC64-HPSC is already behind schedule; cert due late-2026 for Artemis). Full device detail + the BAE RH45/RH12 fab re-onshoring on [[concepts/rha-radiation-hardening]]. So the century's real question is not binary but a **mix ratio**: what share of orbital FLOPs runs on tolerate-and-recover COTS (leading-node, cheap, node-current) vs hardened RISC-V AI MPUs (node-behind, deterministic, mission-critical) — likely COTS for bulk inference/training, hardened parts for the control/safety plane that must not upset.
