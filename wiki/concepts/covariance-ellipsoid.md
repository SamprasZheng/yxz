...
### The dilution region: why a wide covariance is not a low-risk covariance

The under-confident case above is not a symmetric mirror of the over-confident one — it is the more insidious failure, and it has a name. Because [[concepts/pc-probability-of-collision|Pc]] falls to zero as the covariance grows *without bound* (the PDF spread thin over a vast plane leaves negligible mass inside the finite collision disk), scaling the combined covariance by a factor *k* traces a curve that peaks at a **maximum Pc** and then falls off. Once the *actual* covariance exceeds that max-Pc scaling, the event sits in the **dilution region**: the reported Pc is low, but "not because of high certainty but rather because of large ambiguity in the state estimates" ([NASA CARA, NTRS 20190029216](https://ntrs.nasa.gov/citations/20190029216)). The standard deviation at the max-Pc scaling **defines the dilution-region boundary** — so covariance realism and dilution are two faces of the same problem: *a too-wide covariance manufactures a false green.* This is the covariance-input origin of the max-Pc guard and the "presume-risky-until-covariance-tightens" rule detailed in [[concepts/pc-probability-of-collision]] (Layer-Down section), and the structural failure behind the Balch–Martin–Ferson [false-confidence theorem](https://arxiv.org/abs/1706.08565). The practical tell in `cdm_public`: a **missing secondary covariance** is the dilution region taken to its limit — infinite ambiguity — and must never be scored as low-risk.
...
- [[sources/esa-space-environment-report-2026]] — catalog density that stresses covariance realism
