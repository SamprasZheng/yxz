...
> **Usage signal (verified 2026-09-09) — record XCM throughput, but testnet-heavy.** On **2026-09-07** Polkadot reported a **~150% surge in daily transactions** and **record XCM throughput** ("new quarterly highs" for cross-consensus messaging), coinciding with a **~19%/24h DOT move** and a short-liquidation squeeze that carried DOT above **$1** for the first time in 2026 (local high ~$1.03 on 09-07 → ~$1.20 by 09-09). This is the first *usage* data point on this page — the intra-security XCM rails demonstrably moved record volume, i.e. the format works at scale. **Honest discount:** per Chainspect the surge is **largely attributable to the Polkadot Products Devnet** (a public testnet the Polkadot Community Foundation launched July 2026 via **Paseo**), so a large share is **exercised capacity, not paid organic mainnet demand**. Read as "the pipes stress-load and hold," not "durable paying demand found." The [[synthesis/polkadot-interoperability-defi-coretime-app-layer|app-layer synthesis]] carries the full three-dial 2026-09-09 reading. ([cryptonews.net](https://cryptonews.net/news/analytics/33407824/), [coin-turk — activity +150%](https://en.coin-turk.com/polkadot-network-activity-surges-150yuzde-dot-faces-key-price-levels/))
...
- **Hyperbridge** — a cross-chain *coprocessor* (interoperability parachain) using Merkle-Mountain-Range (MMR) state proofs; by Feb 2026 it had processed **>$400M** across Ethereum/Base/Arbitrum/Optimism/Polygon/BNB + Polkadot parachains.
  > **Security caveat (verified, dated):** on **2026-04-13** Hyperbridge's **Ethereum gateway was exploited** — a forged cross-chain message + replayed MMR proofs seized admin rights over the bridged-DOT token contract and **minted ~1 billion fake wrapped DOT** on Ethereum; the **Token Gateway** was subsequently **paused**. This is the canonical illustration of the layer-up thesis: *XCM's internal trust-minimisation does not extend across a bridge — external interop reintroduces bridge risk.* ([Hyperbridge Feb-2026 roundup](https://blog.hyperbridge.network/hyperbridge-february-2026-roundup/), [Polkadot Forum security-audit thread](https://forum.polkadot.network/t/security-audit-tech-debt-remediation-beefy-grandpa-babe-snowbridge-xcm-staking-async-compensation-request/17647))
  > **Update (verified 2026-07-06) — the loss was economic, not the ~1B nominal, and the bridge relaunched:** the ~1B minted DOT was the *nominal* mint; the **actual realised loss was revised to ~$2.5 M** (drained from escrowed pools on Ethereum/Base/BNB/Arbitrum), *up 10×* from the first-day ~$237K estimate (revised 2026-04-16) — i.e. the attacker could only extract a small fraction of the fake supply before liquidity/pauses caught it. Hyperbridge has since **relaunched as an "interoperability hyperstructure"** with a **redesigned architecture that removes the centralised "training wheels"** — proof generation is now **fully permissionless** (any operator can run the open-source **Succinct Prover** and submit valid proofs). Recovery: funds partly traced on-chain to Binance under law-enforcement coordination (est. **months-to-a-year**); if recovery falls short, affected users are to be **made whole in BRIDGE tokens**, and a **"DOT Recovery Loan to Hyperbridge Exploit Victims"** governance pre-proposal is on the Polkadot Forum. The lesson stands and sharpens: *the bridge's own security domain — not the message format — bounds the loss, and remediation is a governance/social-recovery problem once value has left the shared-security domain.* ([BeInCrypto — loss revised to $2.5M](https://beincrypto.com/hyperbridge-exploit-losses-revised-25m/), [Yahoo/CoinDesk — "10x worse"](https://finance.yahoo.com/markets/crypto/articles/hyperbridge-confirms-bridged-polkadot-exploit-053634639.html), [Hyperbridge — relaunch as hyperstructure](https://blog.hyperbridge.network/hyperbridge-relaunches-as-an-interoperability-hyperstructure/), [Polkadot Forum — DOT Recovery Loan pre-proposal](https://forum.polkadot.network/t/updated-pre-proposal-discussion-dot-recovery-loan-to-hyperbridge-exploit-victims/17552))
  > **Update (verified 2026-08-01) — relaunch completed; audit and compensation timeline now concrete.** The redesigned Token Gateway is **back in operation** (June-2026 relaunch), not merely announced: the rebuild was **audited by SRLabs and coordinated by Polytope Labs** (the Hyperbridge/Polkadot dev team), with a **HackenProof rapid-response bug bounty paying >$150K to 20+ researchers in under a week**, and shipped alongside a new **OFT (Omnichain Fungible Token) adapter** for LayerZero-standard assets. The **compensation mechanism and disbursement schedule are to be published 2027-04-13** — exactly one year after the exploit. So the arc is now complete end-to-end: *exploit (2026-04-13) → pause → third-party audit + bounty → permissionless-proving redesign → relaunch (June 2026) → make-whole plan due 2027-04-13.* The remediation corollary holds — recovery from a cross-domain bridge breach is a **multi-quarter governance/social process**, an order of magnitude slower than the seconds it took to mint the fake supply. *(Sources: [Crypto Economy — decentralized upgrades after April hack](https://crypto-economy.com/hyperbridge-relaunches-with-decentralized-upgrades-following-april-hack/); [Technext — hyperstructure + OFT adapter](https://technext24.com/2026/06/16/hyperbridge-relaunches-as-hyperstructure/); [Hyperbridge — recovery and next steps](https://blog.hyperbridge.network/recovery-and-next-steps/), accessed 2026-08-01.)*
