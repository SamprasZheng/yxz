...
| **Flight 14** | V3 / Block 3 (Booster 21 + Ship 41) | NET **Sep 15 → slipped to NET Sep 28, 2026** (~13-day program-schedule slip) from Starbase | pre-flight readiness (Booster 21/Ship 41 static-fire + FAA review) | **Pending** (as of 2026-09-22) — planned **first *orbital* Starship flight test**: first orbital-insertion maneuver, **deploy 26 operational Starlink V3** (up from Flight 13's 20), single-Raptor deorbit burn, controlled reentry + **Pacific-Ocean splashdown** (booster offshore landing-burn in the Gulf). The **NET slip from Sep 15 → Sep 28 is itself the datapoint**: a mid-September→late-September slide with *no mishap open* is a **program-schedule** slip (hardware/readiness), the bounded-by-hardware class distinguished from the investigation-driven tail below — the NET is the earliest observable slip signal (a filed AHA NOTAM confirms an attempt; its absence near NET is an early slip flag per §5). *(Sources: Spaceflight Now / SpaceX / NextSpaceflight, accessed 2026-09-22.)* |
...
- **The T-0 flight-software auto-abort is a distinct slip class the agent must not confuse with a scrub.** Flight 13's 2026-07-16 attempt was aborted by the *vehicle's own avionics* at T-0 (Raptor oxidiser-turbopump anomaly on ~6 engines), not by range/weather/ATC — a **wave-off** in §1 terms. Two operational consequences: (a) the AHA NOTAM stays *valid across the whole multi-attempt cluster* because the backup windows in the E) field already covered Jul 23–24, so an agent sees **no NOTAMR/NOTAMC** between the abort and the eventual launch — the launch-happened signal must come from telemetry/AST, not from a NOTAM state change; and (b) an auto-abort followed by same-hardware re-flight days later (here after "post-abort engine work") carries a *lower* residual slip probability than a mishap-triggered scrub, because the fault was caught pre-ignition with the vehicle intact.
- **Cadence maturity lowers per-flight slip risk.** Block 2's mid-campaign flights (IFT-10, IFT-11) launched near their first NOTAM window, and the **V3 block itself matured fast** — first V3 (Flight 12) was *partial* with a lost booster, second V3 (Flight 13) met *all* mission goals and flew the first operational Starlink V3s. The historical Starship scrub weight (`0.35` in §5 below) is dominated by early-campaign and first-of-type flights (IFT-8, Flight 12) and should be **conditioned on vehicle-block maturity**, not applied as a flat constant.
