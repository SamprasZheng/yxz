...
**Forward trajectory (verified 2026-07-07; extended 2026-09-21):** AIKO pairs `gifted_GENE` (predictive maintenance / anomaly) with **`orbital_OLIVER`**, an *onboard operating system* for autonomous spacecraft operation — the L1-mirror twin evolving toward an on-board L2/L3 planning twin that can act during comms blackout. In 2026 AIKO leads **ASIMOV** — *Autonomous System for In-orbit Mapping and Observation of non-cooperative Vehicles* — a two-year **Italian Space Agency (ASI)**-funded program (consortium: AIKO + Politecnico di Milano + T4i + Tiny Bull Studios) applying this autonomy stack to **rendezvous and proximity operations (RPO)** against **non-cooperative** targets. The non-cooperative framing matters: mapping an *uncooperative* object (no fiducials, no shared state, adversarial or tumbling) is strictly harder than docking with a cooperative one, and it is the highest-consequence closed-loop use of an on-board twin (a wrong prediction risks a collision, not just a missed alert). ASIMOV's GNC fuses vision DNNs for perception with RL for guidance/control into an autonomous onboard autopilot. This is the clearest European datapoint that on-board autonomy is climbing the twin-fidelity ladder from *detect* (GENE) → *operate* (OLIVER) → *map-and-maneuver-in-proximity* (ASIMOV). Funding is modest (€6.5M total, Series A €3.5M Oct 2024), underlining that the European edge here is agency-anchored program access, not capital scale — see [[synthesis/llm-satellite-operations-six-region]].

**Compute-substrate layer (added 2026-09-21):** the software ladder above rides a *hardware* substrate that Europe is now trying to own sovereignly. On **2026-02-09** AIKO announced a partnership with **[[entities/axelera-ai|Axelera AI]]** (Eindhoven; Metis/Europa in-memory-computing inference AIPU) to run `OLIVER`/`GENE` on a European edge accelerator rather than NVIDIA Jetson — the substrate under [[sources/navi-orbital-vlm-earth-obs-2026|NAVI-Orbital]] (Jetson Orin AGX) and Taiwan's [[entities/liscotech|LiscoTech]] GPGPU. Axelera separately partnered with **ESA (2026-03-05) to standardize space-based inference**. This adds a *substrate* row to the on-board-vs-ground split below: the binding limit on climbing the twin ladder is rad-tolerant TOPS-per-watt (open question — SRAM-resident weights are exactly the [[concepts/see-single-event-effects|SEU]]-sensitive structure).
...
The split has a **substrate signature** too — *whose silicon* the on-board twin runs on maps to the same three archetypes: Europe now bids for a sovereign inference chip ([[entities/axelera-ai|Axelera]] in-memory-compute AIPU under AIKO), China runs Huawei Ascend, and the US/Japan/Korea/Taiwan default to NVIDIA Jetson (Orin on [[sources/navi-orbital-vlm-earth-obs-2026|NAVI-Orbital]] and [[entities/liscotech|LiscoTech]]). The on-board twin is therefore gated twice — by rad-hard compute *and* by whose compute — which is why on-board fidelity climbs slowest where the substrate is imported.
...
- [[entities/aiko-space]] — production L1 SDT in GENE; ASIMOV RPO program (non-cooperative mapping)
- [[entities/axelera-ai]] — European on-board inference substrate under AIKO's autonomy stack
