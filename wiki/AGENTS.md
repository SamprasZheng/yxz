...
- `[[synthesis/spacesharks-mission-desk-verification-plan]]` — Verification & test plan for the Mission Desk hackathon entry; seven-layer test pyramid (route probe → parser units → schema+provenance → sandbox-policy chaos → golden-event replay → 24-h soak → final-delivery gate) across four deployment surfaces (Open Cloud / Nemo Cloud / NemoClaw Sandbox / Docker); Nemotron-attestation as the non-negotiable submission gate. Canonical for "how we prove the Mission Desk works on every surface we ship."
