...
- `[[synthesis/spacesharks-mission-desk-verification-plan]]` — Verification & test plan companion to the Mission Desk plan, across four deployment surfaces (Open Cloud / Nemo Cloud / Sandbox / Docker); seven-layer test pyramid (route probe → parser units → schema+provenance → sandbox-policy chaos → golden-event replay → 24-h soak → final-delivery gate); Nemotron-attestation enforced at L1/L3/L7 as the non-negotiable submission gate. Canonical for "is the Mission Desk verified end-to-end before submission."
