...
- `[[synthesis/spacesharks-mission-desk-verification-plan]]` — Seven-layer test pyramid (route probe → parser units → schema+provenance → sandbox-policy chaos → golden-event replay → 24-h soak → final-delivery gate) across four deployment surfaces; Nemotron-attestation enforced at L1/L3/L7 as the non-negotiable submission gate. Canonical for "how we prove the Mission Desk works on every surface shipped."
